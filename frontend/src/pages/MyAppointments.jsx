...
const MyAppointments = () => {
    ...
    const getUserAppointments = async () => {
        try {
            ...
            console.log("Appointments data:", data.appointments) // Log to see actual data structure
        } catch (error) {
            console.log(error)
                ...
                {appointments.map((item, index) => (
                    <div key={index} className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-4 border-b'>
                        ...
                        <div className='flex flex-col gap-2 justify-end text-sm text-center'>
                            ...
                            {/* Chat button - Now always visible for testing */}
                            <Link
                                to={`/chat/${item._id}`}
                                className="bg-blue-600 text-white sm:min-w-48 py-2 rounded hover:bg-blue-700 transition-all duration-300 mt-2"
                            >
                                Chat with Doctor
                            </Link>
                        </div>
